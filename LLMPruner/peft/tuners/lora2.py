...
class LoraLayer:
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            ...
            # self.lora_C.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
            # self.lora_D.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            ...
            # nn.init.zeros_(self.lora_D[adapter_name].weight)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            drop_out = self.lora_dropout[self.active_adapter]
            A = self.lora_A[self.active_adapter]
            B = self.lora_B[self.active_adapter]

            result += B(A(drop_out(x)))

            # C = self.lora_C[self.active_adapter]
            # D = self.lora_D[self.active_adapter]
            # result += D(C(B(A(drop_out(x)))))
