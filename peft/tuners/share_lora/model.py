...
from .config import ShareLoraConfig
from .layer import ShareLoraLayer, Linear
...
class ShareLoraModel(BaseTuner):
    """
    ...
    **Attributes**:
    ...
    prefix: str = "sharelora_"

    def __init__(self, model, config, adapter_name) -> None:
        super().__init__(model, config, adapter_name)

    def _check_new_adapter_config(self, config: ShareLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: ShareLoraConfig, model: nn.Module):
        r"""
    ...
    def find_max_features(self, config: ShareLoraConfig):
        import re
    ...
    def _init_intra_share_modules(self, config: ShareLoraConfig):
        intra_share_modules = []
        r = config.r_intra

        in_features, out_features = 1, 1

        if config.share_mode == "kron":
            in_features, out_features = config.kron_share_size, config.kron_share_size
        elif config.share_mode == "gate":
            in_features, out_features = config.gate_share_size, config.gate_share_size
        elif config.share_mode == "slice":
            in_features, out_features = self.find_max_features(config)
        ...
        for _ in layers:
            ...
            intra_share_modules.append({"lora_A": lora_A, "lora_B": lora_B})

        self.intra_share_modules = intra_share_modules

    def _init_inter_share_modules(self, config: ShareLoraConfig):
        r = config.r_inter
        in_features, out_features = 1, 1
        ...
        self.inter_share_modules = {
            "lora_A": lora_A,
            "lora_B": lora_B
        }

    def _pre_injection_hook(self, model: nn.Module, config: ShareLoraConfig, adapter_name: str):
        self._init_intra_share_modules(config)
        self._init_inter_share_modules(config)
    ...
    def _create_and_replace(
        ...
        _, _, layer_id, _, _ = current_key.split(".")

        intra_module = self.intra_share_modules[int(layer_id)]
        inter_module = self.inter_share_modules

        if isinstance(target, Linear):

            target.update_layer(
                adapter_name,
                r_local=lora_config.r_local,
                r_intra=lora_config.r_intra,
                r_inter=lora_config.r_inter,
            ...
            new_module = self._create_new_module(lora_config, adapter_name, target, intra_module, inter_module)
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                for n, p in model.named_parameters():
                    if "bias" in n:
                        p.requires_grad = True
            elif bias == "sharelora_only":
                for m in model.modules():
                    if isinstance(m, ShareLoraLayer) and hasattr(m, "bias") and m.bias is not None:
                        m.bias.requires_grad = True
            else:
                raise NotImplementedError(f"Requested bias: {bias}, is not implemented.")

    @staticmethod
    def _create_new_module(lora_config, adapter_name, target, intra_module, inter_module, **kwargs):
        new_module = Linear(
            target,
            adapter_name,
            share_mode=lora_config.share_mode,
            r_local=lora_config.r_local,
            r_intra=lora_config.r_intra,
            r_inter=lora_config.r_inter,
            lora_alpha=lora_config.lora_alpha,
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, ShareLoraLayer):
                if module.merged:
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, ShareLoraLayer):
