...
def get_peft_model_state_dict(
    ...
    elif config.peft_type == PeftType.SHARELORA:
        to_return = {k: state_dict[k] for k in state_dict if "sharelora_" in k}
    elif config.peft_type == PeftType.BASELINESHARELORA:
        to_return = {k: state_dict[k] for k in state_dict if "baseline_sharelora_" in k}
    elif config.peft_type==PeftType.TIEDLORA:
        to_return = {k: state_dict[k] for k in state_dict if "tied_lora_" in k}
    else:
...
def set_peft_model_state_dict(
    ...
    if config.peft_type in (
        ...
        parameter_prefix = {
            ...
            PeftType.SHARELORA: "sharelora_",
            PeftType.BASELINESHARELORA:"baseline_sharelora_",
            PeftType.TIEDLORA:"tied_lora_",
        }[config.peft_type]
